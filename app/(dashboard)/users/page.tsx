...
async function getData(q: string): Promise<UserRow[]> {
  ...
  const cookie = (await headers()).get("cookie") ?? ""; // <- auth cookie’ni forward qilamiz
  const url = `${base}/api/users${q ? `?q=${encodeURIComponent(q)}` : ""}`;
  ...
  // HTML qaytsa yoki xato bo‘lsa — bo‘sh ro‘yxat
